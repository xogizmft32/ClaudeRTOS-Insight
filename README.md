...
[![Validation](https://img.shields.io/badge/validation-30%2F30%20PASS-green.svg)](examples/integrated_demo.py)
...
결과  → v2.3 → v5.4.1, 135개 파일, 30/30 검증 통과
```
...
```bash
# 압축 해제 + 의존성 설치
tar -xzf ClaudeRTOS-Insight-v5.3.0.tar.gz
...
# 30/30 Protocol 검증 (하드웨어 불필요, ~100ms)
python3 examples/integrated_demo.py --validate            # 전체 30개
python3 examples/integrated_demo.py --validate --group P  # Parser 10개만
python3 examples/integrated_demo.py --validate --group A  # AI 모듈 10개만
python3 examples/integrated_demo.py --validate --group C  # 분석 10개만

# 패치 적용 (v5.2.0 → v5.4.1)
python3 apply_patch_v521.py --status               # 적용 상태 확인
python3 apply_patch_v521.py --verify               # 패치 필요 여부 확인
python3 apply_patch_v521.py --apply                # 대화형 적용
python3 apply_patch_v521.py --apply --yes          # CI/자동화용 (무확인)
...
| 문서 | 내용 |
|------|------|
| [AI_USAGE_GUIDE_ko.md](docs/03_ai/AI_USAGE_GUIDE.md) | AI 사용 흐름, 캐시, 비용 (한국어) |
| [AI_USAGE_GUIDE.md](docs/03_ai/AI_USAGE_GUIDE.md) | AI 사용 가이드 (영문) |
...
| [AI_PIPELINE_GUIDE.md](docs/03_ai/AI_PIPELINE_GUIDE.md) | **AI 파이프라인 설정 가이드** — 7단계 설정 및 프리셋 |
| [PIPELINE_FLOW.md](docs/04_architecture/PIPELINE_FLOW.md) | **파이프라인 흐름** — 수신→파싱→분석→AI 전체 데이터 흐름 |
| [GEMINI_CLI_GUIDE.md](docs/03_ai/GEMINI_CLI_GUIDE.md) | **Gemini CLI** — 무료 Google AI 분석 |
| [CLAUDE_AGENT_GUIDE.md](docs/03_ai/CLAUDE_AGENT_GUIDE.md) | **Claude Agent SDK** — 에이전트 루프 분석 |
| [CODEX_CLI_GUIDE.md](docs/03_ai/CODEX_CLI_GUIDE.md) | **OpenAI Codex CLI** — gpt-5.3-codex 분석 |
| [AI_PIPELINE_GUIDE.md](docs/03_ai/AI_PIPELINE_GUIDE.md) | **AI 파이프라인 설정 가이드** — 7단계 설정 및 프리셋 |
| [PIPELINE_FLOW.md](docs/04_architecture/PIPELINE_FLOW.md) | **파이프라인 흐름** — 수신→파싱→분석→AI 전체 데이터 흐름 |
| [PATTERN_GUIDE_ko.md](docs/05_quality/PATTERN_GUIDE.md) | 패턴 DB 추가·수정·학습 (한국어) |
| [PATTERN_GUIDE.md](docs/05_quality/PATTERN_GUIDE.md) | Pattern DB 가이드 (영문) |
...
| [SAFETY_AUDIT_SUMMARY.md](docs/05_quality/SAFETY_AUDIT_SUMMARY.md) | 안전성 감사 요약 |
| [SAFETY_DESIGN_GUIDELINES.md](docs/05_quality/SAFETY_AUDIT_SUMMARY.md) | 설계 안전성 지침 |
...
| 문서 | 내용 |
|------|------|
| [CHANGELOG.md](CHANGELOG.md) | 전체 버전 이력 (v2.3 → v5.4.1) |
...
```
Protocol validation:    30/30 PASS  (GROUP P 10/10 · A 10/10 · C 10/10)
