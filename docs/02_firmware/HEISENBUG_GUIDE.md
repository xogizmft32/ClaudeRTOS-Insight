...
# Heisenbug Prevention Guide

> **EN**: A Heisenbug is a bug whose behavior changes when observed (e.g., attaching a debugger makes it disappear). This guide explains how ClaudeRTOS-Insight minimizes observation effects.
...
*Design Principles — How ClaudeRTOS-Insight Minimizes Observation Effects*
...
## 현재 구현의 하이젠버그 방지 요소
*Anti-Heisenbug Mechanisms in the Current Implementation*
...
### 2. 비동기 전송 (ITM SWO)
*Asynchronous Transmission — MonitorTask Writes Separately from Trace Events*
...
*Hardware Timestamps via DWT CYCCNT — ~2 Cycle Read Overhead*
...
*Residual Observation Effects That Cannot Be Fully Eliminated*

현재 구현에도 완전히 제거되지 않은 관측 효과가 있습니다.
**이 항목들을 인지하고 설계해야 합니다.**

### ① ITM 전송 지연 (낮은 ITM 클럭 설정 시)
*ITM Transmission Delay — Rare When SWO Frequency ≥ CPU/4*
...
*Ring Buffer Overflow → Event Drop → Detected as Sequence Gap*
...
### ③ MonitorTask 우선순위 경합
*MonitorTask Priority Contention — Set to tskIDLE_PRIORITY + 1*
...
*CYCCNT Wrap-Around Every ~23.8 Seconds — Auto-Corrected by TimeNormalizer*
...
## 하이젠버그 체크리스트
*Diagnostic Checklist — When a Bug Disappears Under the Debugger*
...
## 진짜 하이젠버그인지 확인하는 방법
*How to Confirm Whether It Is a True Heisenbug*
...
## 임베디드 하이젠버그 주요 패턴
*Common Heisenbug Patterns in Embedded Systems (independent of ClaudeRTOS)*
...
## 페리페럴 관련 하이젠버그
*Peripheral-Related Heisenbugs — Additional Considerations with Peripheral Monitoring*

페리페럴 모니터링 활성화 시 추가로 고려할 사항.

### GPIO 폴링 타이밍
*GPIO Polling Timing — 1 Hz Polling Misses Sub-millisecond Glitches*

`gpio_monitor.c`의 1Hz 폴링은 1ms 이하 글리치를 놓칠 수 있습니다.

```c
// 짧은 글리치(< 1ms)는 샘플링 사이에 발생하고 사라짐
// → 실제로 글리치가 있어도 감지 안 됨
// 대안: EXTI 인터럽트 기반 감지 (정확하지만 오버헤드 증가)
```

**권장**: 글리치 의심 시 EXTI 핀으로 전환 후 EXCCNT로 빈도 측정.

### I2C 모니터 오버헤드
*I2C Monitor Overhead — SR1 Register Polling Impact*

I2C_Monitor_Sample()은 I2C SR1 레지스터를 폴링합니다.
이 레지스터 읽기 자체가 I2C 버스 타이밍에 영향을 줄 수 있습니다 (매우 드뭄).

```c
// 확인: I2C_Monitor_Init 후 버그 재현 여부
// 재현 안됨 → I2C 모니터 오버헤드 의심
// 해결: PeripheralMonitor_Register() 호출 제거 후 재테스트
```
...
```
☐ 1. 페리페럴 모니터 비활성화 후 버그 재현 확인
      PeripheralMonitor_Register() 호출 제거
☐ 2. GPIO 폴링 주기 변경 (1Hz → 0.1Hz) 후 재확인
☐ 3. I2C 오류가 스케줄러 타이밍에 의존하는지 확인
      (태스크 우선순위 변경 후 재현 시 타이밍 버그)
```
